import {
  ...
  const AnalysisResults = () => {
    const [activeTab, setActiveTab] = useState<"emotions" | "elements" | "objects" | "safety">("emotions");
            ...
            <Card className="lg:basis-[65%] shadow-xl h-[calc(100vh-12rem)] overflow-hidden">
              <CardHeader className="pb-4 border-b border-slate-100 dark:border-slate-800">
                <div className="flex items-center justify-between">
                  ...
                  <Button 
                    onClick={() => navigate("/slideshow", { state: { analysis, imageUrls } })}
                    variant="secondary"
                  >
                              ...
                              >
                                ...
                                {emotionMapping.map((emotion, idx) => {
                                  const confidence = mapLikelihoodToConfidence(
                                    face[emotion.key as keyof typeof face] as Likelihood
                                  );
                                  ...
                                  return (
                                    <div key={idx} className="space-y-1 mb-3">
                                      ...
                                      <Progress value={confidence} className={`h-3 ${progressColorClass}`} />
                                    </div>
                                  );
                                })}
                              </div>
                        ...
                        {["adult", "violence", "medical", "spoof"].map((type) => (
                          <div key={type} className="p-3 bg-muted/30 rounded-lg border-2">
                            ...
                            <div className="font-medium capitalize">
                              {analysis.safeSearch?.[type as keyof typeof analysis.safeSearch]?.toLowerCase() || "unknown"}
                            </div>
                          </div>
                        ))}
                      </div>
