...
export default function PinsPage() {
  ...
  const getImageUrl = (imgs: ImageSet): string =>
    imgs["1200x"]?.url || imgs["600x"]?.url ||
    imgs["400x300"]?.url || Object.values(imgs)[0]?.url || "";
  ...
  const confirmImport = async () => {
    ...
    try {
      const imageUrl = getImageUrl(selectedPin.media.images);
      const payload = {
        userId: user.id,
        title,
        imageUrl,
  ...
  if (loading) return <p className="text-center py-20"><Loader/></p>;
  ...
  {/* Glowing ring */}
  <div className="absolute inset-0 rounded-full bg-red-500 blur-sm"></div>
  {/* Your profile image */}
  <img
  ...
  />
    ...
    <p className="text-md text-gray-600">
      Followers: {profile.follower_count} | Following: {profile.following_count} | Pins: {profile.pin_count}
    </p>
...
<h2 className="text-xl font-semibold mb-4 text-center">
  ...
  <span
    className="
      text-3xl
      text-[#E60022]
      drop-shadow-[0_0_12px_rgba(230,0,34,0.8)]
    "
  >
    Pinterest
  </span>{" "}
        ...
        <div className="columns-masonry gap-6">
          {pins.map(pin => (
            <div key={pin.id} className="relative mb-6 group">
              <img
                src={getImageUrl(pin.media.images)}
          ...
          <DialogContent className=" max-w-sm">
            ...
            {selectedPin && (
              <div className="space-y-4">
                <img
                  src={getImageUrl(selectedPin.media.images)}
