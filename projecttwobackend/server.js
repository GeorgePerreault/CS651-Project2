...
async function generateStoryImageWithRetry(sectionText, section, maxAttempts = 3) {
  ...
  const promptVariations = [
    `Generate a vivid illustration for this story section: ${sectionText}. Important: Please create and return an image.`,
    `Create an image showing: ${sectionText}. I need this returned as an image, not text.`,
    `Visualize this scene as an image: ${sectionText}. The output should be an image.`,
    `Draw an illustration for this story: ${sectionText}. Please return an image.`
  ];
...
// Function to generate image using the two-step approach
async function generateImageTwoStep(sectionText, section) {
  try {
    ...
    await delay(1000); // Wait between API calls
    
    // Step 2: Use that description to generate the image
    console.log(`Using description to generate ${section} image`);
    const imagePrompt = `Generate an image based on this description: ${imageDescription}. Important: Please create and return an image, not text.`;
...
app.post('/api/artworks', upload.single('image'), async (req, res) => {
    try {
        ...
        // Ensure we have fallback values in case arrays are empty
        ...
        const emotionalTones = analysis.faces.length > 0
            ...
            // Create genre string for prompt
            const genreDescription = genres.length > 0 
            ? genres.map(g => {
                // Remove reference to genresData and use direct properties
        ...
        try {
            ...
            try {
                if (jsonString) {
                    ...
                    // Alternative approach: fix common JSON formatting issues
                    const fixedText = responseText
                        .replace(/(\w+):/g, '"$1":')  // Add quotes to keys
                        .replace(/:\s*"([^"]*)"([,}])/g, ':"$1"$2')  // Fix value quotes
                        .match(/\{[\s\S]*\}/);
                ...
                // Attempt to extract sections directly using regex
                const extractSection = (section, fallback) => {
                    // Look for quoted strings after the section name
                    ...
                    // Alternative regex to catch different JSON formats
                ...
                // Try a more aggressive extraction approach - get any content between section names
                ...
                if (Object.keys(contentMap).length) {
                    ...
                    // Final fallback - generate sections directly with separate calls
                    analysisStory = {
            ...
            // Create a fallback story using the extracted image elements
            analysisStory = {
                introduction: `A world awash in ${colorDescriptions} came to life as the morning sun rose.`,
                rising_action: `Conflict emerged when ${objectNames} appeared on the horizon.`,
                twist: `Everything changed when they discovered ${webEntities}.`,
        ...
        // Generate images for each section with appropriate strategies
        for (const section of storySections) {
            if (analysis.story[section]) {
                try {
                    ...
                    await delay(1000); // Delay between sections
                    
                    // Use different strategies based on the section
                    // For 'resolution' use the two-step approach as it's most challenging
                    if (section === 'resolution') {
                        ...
                        // If two-step failed, try the retry approach
                        ...
                        // For other sections, use the retry approach
        ...
        // Special handling for resolution image if it's still null
        if (!generatedImages['resolution'] && analysis.story['resolution']) {
            try {
                ...
                const forcedPrompt = `I need an image for this story ending. Please create an image that shows: ${analysis.story['resolution']}. THIS MUST BE RETURNED AS AN IMAGE, NOT TEXT.`;
                await delay(2000); // Longer delay for the final attempt
...
app.get('/api/artworks/history', async (req, res) => {
    try {
        ...
        // Fetch artwork history, sorting by most recent first
        const artworkHistory = await Artwork.find({ userId })
            .sort({ createdAt: -1 })
            .select('title genres createdAt'); // Select only necessary fields

        res.status(200).json(artworkHistory);
    } catch (error) {
...
// Route to fetch a specific artwork's full details
